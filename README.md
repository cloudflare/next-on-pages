...
## Accompanying ESlint Plugin

Alongside the `@cloudflare/next-on-pages` there this repository also includes an additional package `eslint-plugin-next-on-pages` implementing an Eslint plugin which aim is to aid developer at using the `@cloudflare/next-on-pages` more efficiently and improve their overall developer experience when working with it.

For more details check out its [README](./packages/eslint-plugin-next-on-pages/README.md) file.
