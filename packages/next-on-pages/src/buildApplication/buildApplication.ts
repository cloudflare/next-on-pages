...
>) {
	...
	await buildMetadataFiles(outdir);
}
...
async function prepareAndBuildWorker({
	...
	CliOptions,
	'disableChunksDedup' | 'disableWorkerMinification' | 'outdir'
>): Promise<void> {
	...
	if (!/\.vercel\/output\/static\/?$/.test(outdir)) {
	...
	if (!(await validateDir(functionsDir))) {
		...
		generatedFunctionsMaps = await generateFunctionsMap(functionsDir, {
			disableChunksDedup,
			outdir,
		});
	...
	// TODO: await copyVercelStaticAssets(outdir);

	await buildWorkerFile(processedVercelOutput, {
