 ...
 */
export async function buildMetadataFiles(outdir: string) {
	await Promise.all([buildNextStaticHeaders(outdir), buildRoutes(outdir)]);
}

async function buildNextStaticHeaders(outdir: string) {
...
async function buildRoutes(outdir: string) {
	try {
		await writeFile(
